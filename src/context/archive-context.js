...
const ArchiveProvider = ({children}) =>{
    const [archiveState, archiveDispatch] = useReducer(archiveReducer,archiveInitialState);
    ...
    const getAllArchivedNotesHandler = async () => {
      try {
        ...
        if (response.status === 200) {
          archiveDispatch({ type: "UPDATE_ARCHIVE", payload: response.data.archives });
        } else {
  ...
  const moveToArchiveHandler = async (notes) => {
    try {
      ...
      if (response.status === 201) {
        archiveDispatch({ type: "MOVE_TO_ARCHIVE", payload: response.data.archives });
        notesDispatch({
          type: "DELETE_FROM_NOTE",
          payload: response.data.notes,
        });
        toast("Note is moved to archive", { icon: "✔️" });
      } else {
        throw new Error("Can't process the request");
      }
    } catch (error) {
   ...
   const restoreFromArchiveHandler = async (notes) => {
    try {
      ...
      if (response.status === 200) {
        ...
        toast("Note restored from archive", { icon: "✔️" });
      } else {
        throw new Error("Can't process the request");
      }
    } catch (error) {
  ...
  console.log(response)
    ...
    return (
        <ArchiveContext.Provider value={{archiveState, archiveDispatch, getAllArchivedNotesHandler,moveToArchiveHandler,restoreFromArchiveHandler,deleteFromArchiveHandler }}>
            {children}
